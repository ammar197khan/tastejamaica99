...
if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'add_data'){
	...
	$arr['video_link'] = $_REQUEST['video_link'];
	...
	$arr['created_at'] = time();
	$inserted_id = $db->insert($arr, 'jamaica_tv');
	if($inserted_id > 0){
        $imsg->setMessage('Taste Jamaica TV Added Successfully!');
        redirect_header('jamaica-tv.php');
    } else {
...
if(isset($_REQUEST['command']) && $_REQUEST['command'] == 'edit_data'){
	...
	$arr['video_link'] = $_REQUEST['edit_video_link'];
	...
	$arr['created_at'] = time();
	$arr['updated_at'] = time();
	$inserted_id = $db->update($_REQUEST['hidden_id'],$arr, 'jamaica_tv');
	if($inserted_id > 0){
        $imsg->setMessage('Taste Jamaica TV Updated Successfully!');
        redirect_header('jamaica-tv.php');
    } else {
        $imsg->setMessage('Error Occure Try Again', 'error!');
        redirect_header('jamaica-tv.php');
...
if (isset($_REQUEST['command']) && $_REQUEST['command'] == 'delete' && isset($_REQUEST['id']) && intval($_REQUEST['id']) > 0) {
    $result = $db->query("delete from jamaica_tv where id=" . intval($_REQUEST['id']));
    if($result){
        $imsg->setMessage('Taste Jamaica TV Deleted Successfully!');
        redirect_header('jamaica-tv.php');
    } else {
        $imsg->setMessage('Error Occure Try Again', 'error!');
        redirect_header('jamaica-tv.php');
		...
		<div id="content-wrapper" class="d-flex flex-column">
			<div id="content">
				...
				<div class="dashboard_head px-4 pt-2">
					<h4 class="h4 text-dark">Taste Jamaica TV</h4>
					<!-- <div class="dash_text">
				...
				<div class="row px-4 pt-2 mt-4 main_row" style="height: 600px;overflow:scroll">
					...
					<div class="col-lg-12">
						<table class="table table-bordered" id="datatable">
							...
							<tbody>
								<?php
									$sql = 'select * from jamaica_tv where 1=1';
                                    if (!empty($_REQUEST['search'])) {
                                        $sql .= ' AND (title LIKE "%'.$_REQUEST['search'].'%" OR video_link LIKE "%'.$_REQUEST['search'].'%")';
                                    }
                                    // if (!empty($_REQUEST['search_date'])) {
                                    //     $formattedDate = date('Y-m-d', strtotime($_REQUEST['search_date']));
                                    //     $sql .= ' AND DATE_FORMAT(DATE_ADD(created_at, INTERVAL 5 HOUR), "%Y-%m-%d") = "'.$formattedDate.'"';
                                    // }
								...
								?>
									<tr>
										...
										<td>
											...
											<a href="<?php echo 'jamaica-tv.php?command=delete&id='.$row['id']; ?>" onClick="return confirm('Are you sure? You want to Delete This Record?')" class="btn btn-danger btn-sm"><i class="fa-sharp fa-solid fa-trash"></i></a>
														...
														<div class="modal-body">
															<div class="row" style="margin-bottom: 15px;">
																...
																<div class="col-lg-12" style="margin-bottom: 19px;">
																	...
																	<input type="text" maxlength="30" name="edit_title" class="form-control" value="<?= $row['title'] ?>" placeholder="Enter Title">
																</div>
																<div class="col-lg-12" style="margin-bottom: 19px;">
																	<label for="">Video Link</label>
																	<input type="text" name="edit_video_link" class="form-control" value="<?= $row['video_link'] ?>" placeholder="Enter video Link">
																</div>
                                                                <div class="col-lg-12" style="margin-bottom: 19px;">
								...
								<?php }}else{ ?>
                                    <tr>
                                        <td colspan="6" style="text-decoration:underline;text-align:center;font-size:16px;">Sorry! No Record Found!</td>
                                    </tr>
                                <?php } ?>
                                </tbody>
						</table>
					...
					<div class="modal-content">
						...
						<div class="modal-body">
                            <div class="row" style="margin-bottom: 15px;">
                                <div class="col-lg-12" style="margin-bottom: 19px;">
                                    <label for="">Title</label>
                                    <input type="text" maxlength="30" name="title" class="form-control" placeholder="Enter Title">
                                </div>
                                <div class="col-lg-12" style="margin-bottom: 19px;">
                                    <label for="">Video Link</label>
