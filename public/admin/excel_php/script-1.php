...
function p_find_in($table,$str){
	global $link;
	$search_query = "select * from " . $table . " where " . $str . " order by id asc";
	// echo $search_query."<br>";
	$row = mysqli_query($link, $search_query);
	// print_r($row);
	// if($row){
	...
	if ($ext == 'xlsx') {
		...
		foreach ($obj->getWorksheetIterator() as $sheet) {
			...
			for ($i = 0; $i <= $getHighestRow; $i++) {
				...
				$str='name="'.$product_name.'" and unit_id='.intval($stock_name).' and sub_category_id ='.intval($sub_cat_name).' and category_id='.intval($category).' and sku="'.$sku.'"';
				$p_exist=p_find_in('products',$str);
				// $p_exist=0;
					if(intval($p_exist)>0){
						$count = $count+1;
						echo $i.$str.'<br>';
